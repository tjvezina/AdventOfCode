...
namespace TinyJSON
{
	public sealed class Encoder
	{
		...
		bool PrettyPrintEnabled
		{
			get
			{
				return (options & EncodeOptions.PrettyPrint) != 0;
			}
		}
		...
		bool TypeHintsEnabled
		{
			get
			{
				return (options & EncodeOptions.TypeHints) != 0;
			}
		}
		...
		bool IncludePublicPropertiesEnabled
		{
			get
			{
				return (options & EncodeOptions.IncludePublicProperties) != 0;
			}
		}
		...
		bool EnforceHeirarchyOrderEnabled
		{
			get
			{
				return (options & EncodeOptions.EnforceHeirarchyOrder) != 0;
			}
		}
		...
		void EncodeValue( object value, bool forceTypeHint )
		{
			...
			{
				builder.Append( Convert.ToString( ((DateTime)value).Ticks, CultureInfo.InvariantCulture ) );
				return;
			}
		...
		void EncodeObject( object value, bool forceTypeHint )
		{
			...
			// Check for a method with a [BeforeEncode] attribute 
